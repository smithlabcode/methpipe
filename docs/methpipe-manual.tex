...
constitution. With paired-end sequencing experiments, the complements
of these T-rich sequences are also sequenced.  These complementary
sequences have high adenine (A) constitution (A is the complementary
...
The score reported in the fifth column is the $p$-value of a Fisher's
exact test using the frequencies of CC, CT, TC and TT in a $2 \times
2$ matrix. In other words, we test the hypothesis that the observed
allele-specific counts (um and mu, or CT and TC) arise by chance. 
...
The input file format is the \prog{methcounts} output format described
in Section~\ref{sec:estim-methyl-freq}. Once datasets for each
technology (TAB-seq, oxBS-seq and BS-Seq) have been processed and
files \fn{meth\_BS-seq.meth}, \fn{meth\_oxBS-seq.meth} and
\fn{meth\_Tab-seq.meth} are available, the following command will take
all the inputs:
\begin{verbatim}
