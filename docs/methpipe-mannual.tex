...
% \item
% \textbf{-M},\textbf{-max}  \textless integer or float\textgreater : max mismatches (can be fractional)
% \item
% \textbf{-S},\textbf{-output\_stat} \textless string\textgreater : name of output file with methylation statistics
% \item
% \textbf{-v},\textbf{-verbose}  : be verbose; print more run info
% \end{itemize}

% \noindent \\ INPUT: A single file in mapped reads (.mr) format.\\


% \noindent OUTPUT: Output file is in BED format. File with statistics has self-explanatory content\textbf{Address with Andrew}.
% Field \textbf{name} has format \textbf{CpG:X}, where \textbf{CpG} is cytosine context and can be \textbf{CpG, CHH, CHG} (see above), and \textbf{X} is the total number of
% Cs and Ts mapped to the site (i.e. site coverage). Field \textbf{score} is the methylation level.
