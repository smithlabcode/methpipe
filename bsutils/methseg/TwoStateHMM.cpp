...
// static double
// new_parameter(const double param, const vector<double> &obs,
// 	      const vector<double> &p) {
//   const double to_subtract = gsl_sf_psi(param);
//   double numerator = 0;
//   for (size_t i = 0; i < obs.size(); ++i)
//     numerator += p[i]*(gsl_sf_psi(obs[i] + param) - to_subtract);
//   return param*numerator;
// }
...
// 	     const vector<double> &p) {
//   const double p_total = std::accumulate(p.begin(), p.end(), 0.0);
//   //   alpha = inner_product(vals_a.begin(), vals_a.end(), p.begin(), 0.0)/p_total;
//   //   cerr << alpha << endl;
//   //   alpha = accumulate(vals_a.begin(), vals_a.end(), 0.0);
//   //   cerr << alpha << endl;
//   //   beta = inner_product(vals_b.begin(), vals_b.end(), p.begin(), 0.0)/p_total;
//   //   cerr << beta << endl;
//   double prev_alpha = 0.0, prev_beta = 0.0;
//   // alpha = beta = 0.01;
//   while (movement(alpha, prev_alpha) > 1e-5 &&
// 	 movement(beta, prev_beta) > 1e-5) {
...
//     const double to_subtract = gsl_sf_psi(alpha_beta);
//     double denom = 0;
//     for (size_t i = 0; i < vals_a.size(); ++i)
//       denom += p[i]*(gsl_sf_psi(vals_a[i] + vals_b[i] + 
// 				alpha_beta) - to_subtract);
    
//     alpha = new_parameter(prev_alpha, vals_a, p)/denom;
//     beta = new_parameter(prev_beta, vals_b, p)/denom;
//   }
//   lnbeta_helper = gsl_sf_lnbeta(alpha, beta);
// }

// inline static double 
// psi_sum(const vector<double> &p,
// 	const vector<double> &vals, const double param) {
//   double sum = 0.0;
//   for (size_t i = 0; i < vals.size(); ++i)
//     sum += p[i]*gsl_sf_psi_n(1, vals[i] + param);
//   return sum;
// }

// inline static double 
// gradient(const vector<double> &p,
// 	 const vector<double> &both_vals, const double both_params,
// 	 const vector<double> &vals, const double param) {
//   double q = 0.0;
//   for (size_t i = 0; i < both_vals.size(); ++i)
//     q += p[i]*gsl_sf_psi(both_vals[i] + both_params);
//   for (size_t i = 0; i < vals.size(); ++i)
//     q -= p[i]*gsl_sf_psi(vals[i] + param);
//   return q;
// }

// inline static double 
// q_value(const vector<double> &p,
// 	const vector<double> &vals, const double param) {
//   double q = 0.0;
//   for (size_t i = 0; i < vals.size(); ++i)
//     q += p[i]*gsl_sf_psi_n(1, vals[i] + param);
//   return q;
// }

// inline static double 
// z_value(const vector<double> &p,
// 	const vector<double> &vals, const double param) {
//   double z = 0.0;
//   for (size_t i = 0; i < vals.size(); ++i)
//     z += p[i]*gsl_sf_psi_n(1, vals[i] + param);
//   return z;
// }

// inline static double 
// b_value(const double g_alpha, const double g_beta,
// 	const double q_alpha, const double q_beta, const double z) {
//   return (g_alpha/q_alpha + g_beta/q_beta)/(1/z + 1/q_alpha + 1/q_beta);
// }


// static void
// newton_iteration(const vector<double> &p,
// 		 const double p_total,
// 		 const vector<double> &both_vals, 
// 		 const vector<double> &vals_a,
// 		 const vector<double> &vals_b,
// 		 double &alpha, double &beta) {
//   const double both_params = alpha + beta;

//   const double alpha_psi_sum = psi_sum(p, vals_a, alpha);
//   const double beta_psi_sum = psi_sum(p, vals_b, beta);
//   const double both_psi_sum = psi_sum(p, both_vals, both_params);
  
//   const double g_alpha = p_total*(gsl_sf_psi(both_params) - gsl_sf_psi(alpha)) - 
//     // (both_psi_sum - alpha_psi_sum);
//     gradient(p, both_vals, both_params, vals_a, alpha);
//   const double g_beta = p_total*(gsl_sf_psi(both_params) - gsl_sf_psi(beta)) - 
//     // (both_psi_sum - beta_psi_sum);
//     gradient(p, both_vals, both_params, vals_b, beta);
//   const double q_alpha = 
//     // q_value(p, vals_a, alpha) 
//     alpha_psi_sum
//     - p_total*gsl_sf_psi_n(1, alpha);
//   const double q_beta = 
//     // q_value(p, vals_b, beta) 
//     beta_psi_sum
//     - p_total*gsl_sf_psi_n(1, beta);
//   const double z = p_total*gsl_sf_psi_n(1, both_params) - both_psi_sum;//z_value(p, both_vals, both_params);
//   const double b = b_value(g_alpha, g_beta, q_alpha, q_beta, z);
//   alpha -= 1.5*(g_alpha - b)/q_alpha;
//   beta -= 1.5*(g_beta - b)/q_beta;
// }
...
// 	     const vector<double> &p) {
//   const double p_total = std::accumulate(p.begin(), p.end(), 0.0);
//   //   alpha = accumulate(vals_a.begin(), vals_a.end(), 0.0)/vals_a.size();
//   //   beta = accumulate(vals_b.begin(), vals_b.end(), 0.0)/vals_b.size();
//   double prev_alpha = 0.0, prev_beta = 0.0;
  
//   vector<double> both_vals;
//   std::transform(vals_a.begin(), vals_a.end(), vals_b.begin(), 
// 		 back_inserter(both_vals), std::plus<double>());

//   size_t count = 0;
//   while (movement(alpha, prev_alpha) > tolerance &&
// 	 movement(beta, prev_beta) > tolerance && count < 3) {
//     prev_alpha = alpha;
//     prev_beta = beta;
//     newton_iteration(p, p_total, both_vals, vals_a, vals_b, alpha, beta);
//     cerr << alpha << "\t" << movement(alpha, prev_alpha) << "\t" 
// 	 << beta << "\t" << movement(beta, prev_beta) << endl;
//     ++count;
//   }
//   lnbeta_helper = gsl_sf_lnbeta(alpha, beta);
// }
...
TwoStateHMMB::BaumWelchTraining(const vector<pair<double, double> > &values,
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    vals_a[i] = // values[i].first;
      log(std::min(std::max(values[i].first/(values[i].first + values[i].second), 1e-2),
		   1.0 - 1e-2));
    vals_b[i] = // values[i].second;
      log(1 - std::min(std::max(values[i].first/(values[i].first + values[i].second), 1e-2),
		       1.0 - 1e-2));
  }
  ...
  for (size_t i = 0; i < max_iterations; ++i) {
	...
	// << setw(14) << (total - prev_total)/total
// 	   << setw(14) << fabs(total - prev_total) 
	   << endl;
    }
