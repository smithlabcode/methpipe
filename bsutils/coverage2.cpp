...
static void
get_coverage(const vector<SimpleGenomicRegion> &mapped,
			 const size_t start,
			 const size_t end,
			 std::ostream &out) 
{
		...
		for (size_t i = 0; i < ends.size(); ) // i is chaned below
		{
...
static void
get_coverage_wig(const vector<SimpleGenomicRegion> &mapped,
				 const size_t start,
				 const size_t end,
				 std::ostream &out) 
{
		...
		try 
		{
				...
				for (size_t i = 1; i < changes.size(); ++i) 
				{
						...
						if (PRINT_WIG)
								...
								get_coverage(mapped, prev, changes[i], *out);
						prev = changes[i];
				}
