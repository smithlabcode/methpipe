...
struct bernoulli 
{
		...
		bernoulli(const double p) : prob(p) {}
		double operator()(const size_t &val) const;
...
void
bernoulli::fit(const vector<size_t> &vals, const vector<double> &p) 
{
...
TwoStateHMM::forward_algorithm(const vector<size_t > &vals,
								...
								const double lp_bt,
								const bernoulli &fg_distro,
								const bernoulli &bg_distro,
...
TwoStateHMM::BaumWelchTraining(const vector<size_t > &widths,
								...
								double &fg_prob,
								double &bg_prob) const 
{
  
		bernoulli fg_distro(fg_prob);
		bernoulli bg_distro(bg_prob);
		...
		const double score = BaumWelchTraining(widths, reset_points,
											   start_trans[0], start_trans[1],
											   trans[0][0], trans[0][1], end_trans[0],
											   trans[1][0], trans[1][1], end_trans[1],
											   fg_distro, bg_distro);
  
		fg_prob = fg_distro.prob;
		bg_prob = bg_distro.prob;
...
TwoStateHMM::PosteriorScores(const vector<size_t > &values,
		...
		const bernoulli fg_distro(fg_lambda);
		const bernoulli bg_distro(bg_lambda);
