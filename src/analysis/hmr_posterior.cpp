...
main(int argc, const char **argv) {
  ...
  try {
    ...
    OptionParser opt_parse(strip_path(argv[0]), "Program for computing "
			   "single CpG site posterior probability of "
                           "being inside an HMR", "<cpg-BED-file>");
    ...
    if (VERBOSE)
      cerr << "4th column format:  CpG:<M_reads>:<U_reads>" << endl;
    for (size_t i = 0; i < cpgs.size(); ++i) {
