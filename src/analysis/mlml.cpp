...
parse_line(const bool REV, const string &line,
  ...
  if (is_methpipe_file)
    ...
    is >> chr >> pos >> dummy >> str_count >> level;
    count = 
...
main(int argc, const char **argv) {
  ...
  try {
    ...
    if (!hydroxy_file.empty() && !bs_seq_file.empty() && !oxbs_seq_file.empty()) {
      ...
      bool hydroxy_format, bs_seq_format, oxbs_seq_format;
      hydroxy_format = methpipe::is_methpipe_file_single(hydroxy_file);
      bs_seq_format = methpipe::is_methpipe_file_single(bs_seq_file);
      oxbs_seq_format = methpipe::is_methpipe_file_single(oxbs_seq_file);
	...
	if ((h + g > 0 && u + t > 0 ) ||
      ...
      bool f_format, s_format;
      if (oxbs_seq_file.empty()) {
	s_rev = true;
	f_in.open(hydroxy_file.c_str());
	s_in.open(bs_seq_file.c_str());
        f_format = methpipe::is_methpipe_file_single(hydroxy_file);
        s_format = methpipe::is_methpipe_file_single(bs_seq_file);
      }
      else if (hydroxy_file.empty()) {
	f_rev = true;
	f_in.open(bs_seq_file.c_str());
	s_in.open(oxbs_seq_file.c_str());
        f_format = methpipe::is_methpipe_file_single(bs_seq_file);
        s_format = methpipe::is_methpipe_file_single(oxbs_seq_file);
      }
      else {
	f_in.open(oxbs_seq_file.c_str());
	s_in.open(hydroxy_file.c_str());
        f_format = methpipe::is_methpipe_file_single(oxbs_seq_file);
