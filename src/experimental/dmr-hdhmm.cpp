...
static const string STATE_LABEL_STRS_ARRAY[] = {"GAIN", "SAME", "LOSS"};

static const vector<string>
STATE_LABEL_STRS(STATE_LABEL_STRS_ARRAY, STATE_LABEL_STRS_ARRAY + 3);
    ...
    if (cpgs_a[i].same_chrom(cpgs_b[j]) &&
      ...
      if (m_a + u_a > 0.0 && m_b + u_b > 0.0) {
        const double diffscore =
          ContingencyTable::beta_population_greater(m_b + pseudocount,
                                                    u_b + pseudocount,
                                                    m_a + pseudocount,
                                                    u_a + pseudocount);
...
static void
calcualte_domain_p_values_by_wilcoxon_test(const vector<SimpleGenomicRegion> &cpgs,
                                           const vector<size_t> &meth_a,
                                           const vector<size_t> &unmeth_a,
                                           const vector<size_t> &meth_b,
                                           const vector<size_t> &unmeth_b,
  ...
  for (size_t i = 0; i < domains.size(); ++i) {
    ...
    while (j < cpgs.size() && simdom.contains(cpgs[j])) {
      methylation_a.push_back(static_cast<double>(meth_a[j]) / (meth_a[j] + unmeth_a[j]));
      methylation_b.push_back(static_cast<double>(meth_b[j]) / (meth_b[j] + unmeth_b[j]));
      ++domain_size;
      ++j;
    }
...
static void
score_domain_by_wilcoxon_test(const vector<SimpleGenomicRegion> &cpgs,
  ...
  vector<double> p_values;
  calcualte_domain_p_values_by_wilcoxon_test(cpgs, meth_a, unmeth_a, meth_b, unmeth_b, domains, p_values);
...
static void
calculate_random_scores_from_background(const vector<size_t> &meth_a,
                                        const vector<size_t> &unmeth_a,
                                        const vector<size_t> &meth_b,
                                        const vector<size_t> &unmeth_b,
...
public:
  bool operator()(const GenomicRegion &lhs,
                  const GenomicRegion &rhs) const {
...
static void
score_domain_by_diff_meth_emp_p_value(const vector<SimpleGenomicRegion> &cpgs,
                                      const vector<size_t> &meth_a,  const vector<size_t> &unmeth_a,
                                      const vector<size_t> &meth_b,  const vector<size_t> &unmeth_b,
                                      const double fdr, double fdr_cutoff, vector<GenomicRegion> &domains,
                                      const bool VERBOSE) {
  ...
  for (size_t i = 0; i < domains.size(); ++i) {
    ...
    if (domain_cpg_size < sz) {
      ...
      domain_cpg_size = sz;
      random_scores.clear();
      calculate_random_scores_from_background(meth_a, unmeth_a, meth_b, unmeth_b, domain_cpg_size,
