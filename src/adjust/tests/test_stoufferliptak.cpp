...
TEST(StoufferLiptak, WorksWithoutCorrelationMatrix) {
  ...
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0.0168, max_abs_error));
  
  p_vals = {0.5, 0.5, 0.5, 0.5, 0.5};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0.5, max_abs_error));

  p_vals = {0.5, 0.1, 0.5, 0.5, 0.5};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0.283, max_abs_error));

  p_vals = {0.5, 0.1, 0.1, 0.1, 0.5};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0.042, max_abs_error));

  p_vals = {1, 0.1, 0.1};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0.748, max_abs_error));
  
  p_vals = {1, 1, 1};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(1.0, max_abs_error));
  
  p_vals = {0, 0};
  ASSERT_THAT(stouffer_liptak(p_vals), DoubleNear(0, max_abs_error));
}
...
TEST(StoufferLiptak, WorksWithPositiveDefiniteCorrelationMatrix) {
  ...
  ASSERT_THAT(stouffer_liptak(pvals, corr_matrix), 
